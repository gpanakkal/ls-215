/**
 * Re-implement sentiment analysis with regex-based positive and negative word lists.
 */
...
function sentiment(text) {
  const positives = [...text.match(positiveRegex)].map((word) => word.toLowerCase());
  const positiveCount = positives.length;
  const negatives = [...text.match(negativeRegex)].map((word) => word.toLowerCase());
